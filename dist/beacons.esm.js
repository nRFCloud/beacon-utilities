    ...
    while (currentPos < scanRecord.length) {
        ...
        switch (fieldType) {
            ...
            case AdvertisementDataType.DATA_TYPE_SERVICE_DATA:
                var bytes = extractBytes(scanRecord, currentPos, UUIDLength.UUID_BYTES_16_BIT);
                var serviceDataUuid = '' + ((bytes[1] & 0xFF).toString(16)) + (bytes[0] & 0xFF).toString(16);
                var serviceDataArray = extractBytes(scanRecord, currentPos + UUIDLength.UUID_BYTES_16_BIT, dataLength - UUIDLength.UUID_BYTES_16_BIT);
...
function isIBeacon(data) {
    if (typeof data.manufacturerData !== 'undefined') {
        if (typeof data.manufacturerData[Manufacturer.Apple] !== 'undefined') {
            data = data.manufacturerData[Manufacturer.Apple];
        }
        else {
            return false;
        }
...
function isAltBeacon(data) {
    if (typeof data.manufacturerData !== 'undefined') {
        if (typeof data.manufacturerData[Manufacturer.Radius] !== 'undefined') {
            data = data.manufacturerData[Manufacturer.Radius];
        }
        else {
            return false;
        }
